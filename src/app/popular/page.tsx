...
export default function PopularPage() {
  ...
  return (
    <div className={`w-full px-4 py-12 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
      ...
      <div className={`max-w-3xl mx-auto text-center mb-12 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
      ...
      <div className="space-y-6">
        {popularPodcasts.map((podcast, index) => (
          <Link key={podcast.id} href={`/podcasts/${podcast.id}`} className="group">
            <div className={`flex flex-row gap-6 p-6 rounded-lg hover:shadow-md transition-shadow ${isDark ? 'bg-gray-950' : 'bg-white'}`}>
